  ...
  return (
    <ClassPanelShell classId="middle">
      ...
      <StorageGrid
        ...
        onAdjust={(k, d) =>
          apply(`middle.storage.${k} ${d}`, (s) => {
            const st = s.classes.middle.storage as unknown as Record<string, number>;
            st[k] = Math.max(0, (st[k] ?? 0) + d);
          })
        }
        onSet={(k, v) =>
          apply(`middle.storage.${k} = ${v}`, (s) => {
            const st = s.classes.middle.storage as unknown as Record<string, number>;
            st[k] = Math.max(0, v);
          })
        }
      ...
      <div>
        ...
        <ul className="space-y-1 text-xs">
          {m.companies.map((c) => (
            <li key={c.id} className="rounded-md border border-slate-800 bg-slate-950/40 px-3 py-2">
              <div className="flex items-center justify-between">
                <span className="text-slate-200">{c.label}</span>
                <span className="text-slate-500">L{c.wageLevel} · {c.workersAssigned}M / {c.workingClassEmployees}W</span>
              </div>
            </li>
          ))}
          {m.companies.length === 0 ? <li className="text-slate-500">No companies built.</li> : null}
        </ul>
      </div>
      <div>
